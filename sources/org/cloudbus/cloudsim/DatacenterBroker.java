...
public class DatacenterBroker extends SimEntity {
	...
	protected void submitCloudlets() {
		...
		{
			Cloudlet Cloudlet_Small = temp.get(0);
			for(Cloudlet Cloudlet_Check : temp)
			{
				if (Cloudlet_Small.getCloudletLength() > Cloudlet_Check.getCloudletLength())
				{
					Cloudlet_Small= Cloudlet_Check;
				}
			}
			SJF.add(Cloudlet_Small);
			temp.remove(Cloudlet_Small);
		}
		
		for(Cloudlet cloudlet: SJF)
		{
			temp.add(cloudlet);
		}
		...
		for(int i = 0; i < size; i++) {
			...
			// HSLJF PLUS+
		 	...
		 	for(int j = 0; j < getVmsCreatedList().size(); j++) {
		 		...
		 		double ET = selectedTask.getCloudletLength() / vm.getMips();
		 		double LoadVm = 0;
		 		for(int m = 0; m < temp.size(); m++) {
		 			if (temp.get(m).getVmId() == vm.getId()) {
		 				LoadVm += temp.get(m).getCloudletLength() / vm.getMips();
	 				}
		 		}
		 		CT_VMs[j] = ET + LoadVm;
		 		selectedVm = vm;
	        ...
	        for (int n = 0; n < CT_VMs.length; n++) {
            	...
            	if (CT_VMs[minIndex] == CT_VMs[n]) {
            		...
            		if (getVmsCreatedList().get(minIndex).getMips() > getVmsCreatedList().get(n).getMips()) {
            			...
            			selectedVm = getVmsCreatedList().get(n);
            			CTM[i][n] = CT;
            		}
            	}
            	else {
